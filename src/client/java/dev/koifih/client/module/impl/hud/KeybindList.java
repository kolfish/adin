...
public final class KeybindList extends HudModule {
    ...
    private void onHudRender(HudRenderEvent event) {
        ...
        Rects.draw(graphics, x, y, boxWidth, height, Math.round(RADIUS * scale), Colors.withAlpha(Theme.MAIN, ALPHA));
        Text.drawCentered(graphics, title, x + padding, y + header * 0.5f, textSize, Theme.TEXT);
        float headerIcon = HEADER_ICON_WIDTH * scale;
        Icons.draw(graphics, KEYBOARD_ICON, right - padding - headerIcon, y + (header - headerIcon) * 0.5f, headerIcon, Theme.TEXT);
        Rects.draw(graphics, x, y + header, boxWidth, Math.max(1, Math.round(DIVIDER * scale)), 0, Theme.UNCHECKED);
        ...
        for (Module module : bound) {
            ...
            Icons.draw(graphics, module.hold() ? HOLD_ICON : TOGGLE_ICON, keyX - gap - icon, centerY - icon * 0.5f, icon, Theme.MUTED);
