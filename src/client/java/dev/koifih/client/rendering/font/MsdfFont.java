...
public final class MsdfFont {
    ...
    private MsdfFont(Metrics metrics, Identifier texture, int fallbackCodepoint) {
    ...
    public static MsdfFont load(String name, Identifier texture, int fallbackCodepoint) {
        String path = METRICS_DIRECTORY + name + ".json";
        try (var stream = MsdfFont.class.getResourceAsStream(path)) {
            if (stream == null) throw new IllegalStateException("Missing font metrics: " + path);
            try (var reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
                return new MsdfFont(new Gson().fromJson(reader, Metrics.class), texture, fallbackCodepoint);
            }
